...
{
    std::shared_ptr<scene_loader_creator> cr = scene_load_solution().get_creator();
    src_loader = std::shared_ptr<base_scene_loader>(cr->create_loader());
}

std::shared_ptr<scene> scene_secretary::load(std::string name)
{
    src_loader->open(name);
