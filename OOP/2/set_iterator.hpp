...
T &set_iterator<T>::operator *()
{
    if (cur.expired())
    {
        ...
        throw set_iterator_exception(ctime(&timenow), __FILE__, typeid(set_iterator).name(), __FUNCTION__);
    }
...
template <typename T>
set_iterator<T> &set_iterator<T>::operator +=(long shift)
{
...
template <typename T>
set_iterator<T> set_iterator<T>::operator +(long shift) const
{
    auto it(*this);
    it += shift;
    return it;
