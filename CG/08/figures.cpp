...
void cut_fig::paint(QPainter &painter) const
{
    painter.setPen(QPen(fg));
    for (auto edge : edges)
...
line cut_fig::get_edge(long index)
{
    return edges[index];
}

long cut_fig::sign(long num)
{
    if (!num)
        return 0;
    return (num > 0) ? 1 : -1;
...
void cut_fig::add_point(point point)
{
...
void cut_fig::change_last(point point)
{
    points[points.size() - 1] = point;
    if (points.size() > 1)
        edges[edges.size() - 1].set_p1(point);
}

void cut_fig::end_fig()
{
    points.pop_back();
    if (points.size() > 1)
